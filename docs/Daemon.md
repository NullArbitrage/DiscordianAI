...
> **Important**: Cron environments have minimal `PATH`. If you use pyenv or linuxbrew Python, make sure your `PATH` is set in the crontab or in a wrapper script. The launcher checks pyenv automatically, but pyenv must be on `PATH` for `command -v pyenv` to find it.

## Running with systemd

For automatic restart on crash and proper service management, use the included systemd template unit.
...
2. **Edit the copy** — replace `/path/to/DiscordianAI` with your actual installation path, and adjust `PATH` to include pyenv or linuxbrew if needed.
...
3. **Reload systemd and enable**:
   ...
   The instance name (after `@`) becomes the config file name. `discordian-bot@bot` uses `bot.ini`, `discordian-bot@production` uses `production.ini`.

4. **Check status**:
   ```bash
   systemctl --user status discordian-bot@bot
   ```

5. **View logs**:
   ```bash
   journalctl --user -u discordian-bot@bot -f
   ```

   Logs are also appended to `bot.log` in the project directory (via `StandardOutput=append:`).
...
2. **Install** the systemd unit as described above
3. **Enable and start**: `systemctl --user enable --now discordian-bot@bot`
...
- Check the resolution order (venv → pyenv → system) and ensure your preferred Python is first